...
verify_sources() {
  ...
  ok "Found $agent_count agents, $command_count commands, $hook_count hooks"
}
...
backup_existing() {
  ...
  for target in "$AGENTS_DIR" "$COMMANDS_DIR" "$HOOKS_DIR" "$SETTINGS_FILE"; do
    if [ -e "$target" ]; then
      needs_backup=1
      break
    fi
...
uninstall() {
  ...
  ok "Removed agents, commands, hooks, and settings"
